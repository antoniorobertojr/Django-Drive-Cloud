...
class PermissionCopyTest(TestCase, UserMixin, FolderMixin, FileMixin, ShareMixin):
    def setUp(self):
        ...
        self.child_folder = self.create_folder(
            "ChildFolder", self.user1, parent=self.parent_folder
        )
    ...
    def test_copy_folder_permissions_to_file(self):
        ...
        shares = Share.objects.filter(
            content_type=file_content_type, object_id=self.file.id
        )
    ...
    def test_copy_parent_folder_permissions_to_subfolder(self):
        ...
        child_shares = Share.objects.filter(
            content_type=child_folder_content_type, object_id=self.child_folder.id
        )

        self.assertEqual(child_shares.count(), 1)


class PermissionPropagationTest(
    TestCase, UserMixin, FolderMixin, FileMixin, ShareMixin
):
    def setUp(self):
        self.user1 = self.create_user("user1", "password123")
        self.user2 = self.create_user("user2", "password123")
        ...
        self.file_in_child_folder = self.create_file(
            "FileInChild", self.child_folder, self.user1
        )
